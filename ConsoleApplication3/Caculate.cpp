...
#include <ctype.h>
#include <cstdlib> 
#include "unitmanager.h"// For atof
...
void replaceString(char* source, const char* target, double value) {
    char buffer[150] = {};
    ...
    while (1) {
        ...
        // Add the value
        insert_point += sprintf(insert_point, "%f", value);
...
const char* CalculateEx(const char* string)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            std::pair<const char*, double> p = UnitManager::Get()->returnPlayer().GetElement(i);
